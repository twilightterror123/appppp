...
async function render(){
 ...
 src.connect(lowCut).connect(low).connect(mid).connect(high).connect(comp).connect(g).connect(master).connect(off.destination);
...
$("sourceA").onclick=()=>{$("sourceAudio").currentTime=0;$("sourceAudio").play()};$("resultB").onclick=()=>{$("resultAudio").currentTime=0;$("resultAudio").play()};
